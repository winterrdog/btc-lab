...
ExtendedKey deserialize_key(const byte_array_t& data) {
  ExtendedKey key{};
  const u8* next = data.data();

  // version (4 bytes)
  std::memcpy(key.version.data(), next, 4);
  next += key.version.size();

  // depth (1 byte)
  std::memcpy(&key.depth, next, 1);
  next += sizeof(u8);

  // fingerprint (4 bytes)
  std::memcpy(key.fingerprint.data(), next, 4);
  next += key.fingerprint.size();

  // index (4 bytes)
  std::memcpy(key.index.data(), next, 4);
  next += key.index.size();

  // chaincode (32 bytes)
  std::memcpy(key.chaincode.data(), next, 32);
...
sha256_hash_t tagged_hash(std::span<const u8> data, const std::string& tag) {
  // sha256 hash the tag to get hashA
  sha256_hash_t hashA{};  // hash of the input tag
  byte_array_t tag_vec(tag.begin(), tag.end());
  sha256(tag_vec, hashA.data());
...
Bip32Key derive_priv_child(secp256k1_context* ctx,
  ...
  if (hardened || index >= 0x80000000) {
    ...
    byte_array_t chaincode_bytes{chaincode.begin(), chaincode.end()};
  ...
  byte_array_t tweak(hmac.begin(), hmac.begin() + 32);
...
witness_program_t get_p2tr_keypath_program(secp256k1_context* ctx,
  ...
  sha256_hash_t tap_tweak{tagged_hash(internal_x_bytes, "TapTweak")};
...
State recover_wallet_state(secp256k1_context* ctx, const std::string& xprv) {
...

  {
    ...
    byte_array_t xprv_bytes{base58_decode(xprv)};
  ...
  std::unordered_set<std::string> tgt_witness_hexes{};
  {
    ...
    for (const auto& program : state.programs) {
      bytes = byte_array_t(program.begin(), program.end());
