...
#include <array>
#include <cmath>
#include <compare>  // spaceship operation
#include <cstring>
#include <iostream>
#include <map>
#include <memory>
...
using byte_array_t = std::vector<u8>;

using json = nlohmann::json;

using bytes36 = std::array<u8, 36>;
using utxo_key_t = bytes36;
using outpoint_ser_t = bytes36;

using bytes32 = std::array<u8, 32>;
using txid_t = bytes32;
using chaincode_t = bytes32;
using secret_key_t = bytes32;
using sha256_hash_t = bytes32;

using bytes64 = std::array<u8, 64>;
using sha512_hash_t = bytes64;
using schnorr_sig_t = bytes64;

using pub_key_t = std::array<u8, 33>;
using witness_program_t = std::array<u8, 34>;

constexpr u8 SHA256_HASH_LENGTH = 32;

json fetch_300_blocks_to_json();  // todo: to be removed
...
class BitcoinAmt {
 public:
  ...
  BitcoinAmt operator-(const BitcoinAmt& other) const;
  std::strong_ordering operator<=>(const BitcoinAmt& other) const;

  // required by compiler to handle == or != cases
  bool operator==(const BitcoinAmt& other) const = default;
...
struct Hex {
  ...
  static byte_array_t Decode(const std::string& hex) {
    if (hex.length() % 2 != 0)
  ...
  static std::string Encode(std::span<const u8> bytes) {
    std::string result{};
...
struct Outpoint {
  ...
  static Outpoint from_str(const std::string& txid_hex, u32 vout) {
    Outpoint outpoint;

    outpoint.vout = vout;

    byte_array_t v = Hex::Decode(txid_hex);
    ...
    std::ranges::reverse(outpoint.txid);
...
struct Utxo {
  ...
  BitcoinAmt value;
  witness_program_t scriptpubkey;
};
