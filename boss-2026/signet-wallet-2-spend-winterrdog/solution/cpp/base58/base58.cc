...
std::string base58_encode(const byte_array_t& input) {
  ...
  std::string result{};
  byte_array_t temp = input;  // mutate local copy not original data
...
byte_array_t base58_decode(const std::string& b58) {
  ...
  byte_array_t final_output(leading_zeros, 0x0);
  ...
  byte_array_t checksum(final_output.end() - 4, final_output.end());

  u8 hash[SHA256_HASH_LENGTH]{};
  byte_array_t original(final_output.begin(), final_output.end() - 4);
...
static byte_array_t prep_base58_payload(const byte_array_t& original) {
  u8 hash[SHA256_HASH_LENGTH]{};
  HASH256(original, hash);

  // append first 4 bytes of hash
  byte_array_t payload = original;
  for (int i = 0; i < 4; ++i)
...
void print_hex(const std::string& label, const byte_array_t& bytes) {
  std::cout << label << ": ";
  for (u8 b : bytes) {
...
int main() {
  try {
    ...
    // 2. Test Decoding
    byte_array_t decoded = base58_decode(b58_address);
